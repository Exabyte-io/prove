...
export function ElementalReferenceEnergies({ property, title }) {
    var _a, _b;
    const rows = (_b = (_a = property.value) === null || _a === void 0 ? void 0 : _a.rows) !== null && _b !== void 0 ? _b : [];
    if (!rows.length) {
        return null;
    }
    return (_jsxs(Box, { p: 4, sx: { overflow: "auto" }, children: [_jsx(Typography, { variant: "subtitle2", color: "text.primary", gutterBottom: true, children: title || "Formation energy verification" }), _jsxs(Table, { size: "small", children: [_jsx(TableHead, { children: _jsxs(TableRow, { children: [_jsx(TableCell, { children: "Material" }), _jsx(TableCell, { align: "right", children: "Total energy (eV)" }), _jsx(TableCell, { align: "right", children: "N atoms" }), _jsx(TableCell, { align: "right", children: "Total energy per atom (eV/atom)" }), _jsx(TableCell, { children: "Precision" })] }) }), _jsx(TableBody, { children: rows.map((row) => (_jsxs(TableRow, { children: [_jsx(TableCell, { children: row.label }), _jsx(TableCell, { align: "right", children: formatNumber(row.total_energy, 6) }), _jsx(TableCell, { align: "right", children: formatNumber(row.n_atoms, 0) }), _jsx(TableCell, { align: "right", children: formatNumber(row.total_energy_per_atom, 3) }), _jsx(TableCell, { children: formatPrecision(row) })] }, row.label))) })] })] }));
}
